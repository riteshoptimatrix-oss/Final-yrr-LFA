import { Browser, BrowserContext, Page, Request, chromium } from 'playwright';
import { logger } from '../../utils/logger';
import { existsSync, mkdirSync } from 'fs';
import path from 'path';

const PERSISTENT_PROFILE_DIR = path.resolve(process.cwd(), '.gm-user-data');
const CHROMIUM_ARGS = [
...
interface PageDiagnostics {
  console: string[];
  requests: Array<{ url: string; method: string; status?: number; resourceType: string }>;
}
...
const countryLocationMap: Record<string, { lat: number; lng: number; tz: string; locale: string; lang: string }> = {
...
export class BrowserManager {
  ...
  private pageOwner = new Map<Page, ManagedContext>();
  private pageDiagnostics = new WeakMap<Page, PageDiagnostics>();
  ...
  private maxLaunchAttempts = 5;
  private lastLaunchAttemptTime = 0;
  private backoffResetWindowMs = 120000;
  ...
  constructor() {
    ...
    logger.info({ maxContexts: MAX_CONTEXTS, maxPagesPerContext: MAX_PAGES_PER_CONTEXT, maxTotalPages: MAX_TOTAL_PAGES }, 'BrowserManager: Initialized');
  }
  ...
  async acquire(sourceName: string, _browserType?: string): Promise<{ page: Page; browser: Browser; context: BrowserContext }> {
  ...
  async acquireForCountry(sourceName: string, country: string): Promise<{ page: Page; browser: Browser; context: BrowserContext }> {
    ...
    try {
      ...
      page.setDefaultNavigationTimeout(PAGE_TIMEOUT_MS);
      await this.setupPage(page);
      const managed: ManagedContext = { context: ctx, pages: new Set([page]), lastUsed: Date.now() };
      this.contexts.push(managed);
      this.pageOwner.set(page, managed);
      this.totalPagesCreated++;
      logger.debug({ source: sourceName, country, locale: loc.locale, tz: loc.tz }, 'BrowserManager: Page acquired with country context');
  ...
  async acquireMultiple(sourceName: string, count: number): Promise<{ page: Page; browser: Browser; context: BrowserContext }[]> {
    ...
    try {
      ...
      for (const p of pages) {
        try { await this.release(p.page, sourceName); } catch {}
      }
      throw err;
    }
    return pages;
  ...
  async acquireFresh(sourceName: string): Promise<{ page: Page; browser: Browser; context: BrowserContext }> {
    const unlock = await this.lock();
    try {
      const browser = await this.ensureBrowserLocked();
      let ctx = this.contexts.find((c) => c.pages.size < MAX_PAGES_PER_CONTEXT);
      if (!ctx) ctx = await this.createContextLocked();
      const page = await ctx.context.newPage();
      page.setDefaultTimeout(PAGE_TIMEOUT_MS);
      page.setDefaultNavigationTimeout(PAGE_TIMEOUT_MS);
      await this.setupPage(page);
  ...
  async release(page: Page, _sourceName: string): Promise<void> {
    const unlock = await this.lock();
    try {
      ...
      logger.debug({ remainingPages: ctx.pages.size, totalClosed: this.totalPagesClosed }, 'BrowserManager: Page released');
    } finally {
      unlock();
    }
  }

  async releaseAll(pages: Page[], sourceName: string): Promise<void> {
    for (const p of pages) {
      try { await this.release(p, sourceName); } catch {}
    }
  }

  async releaseAllActive(): Promise<void> {
    const unlock = await this.lock();
    try {
  ...
  async reset(): Promise<void> {
    ...
    await this.shutdown();
    this.startCleanupTimer();
    logger.info({}, 'BrowserManager: Reset complete — browser closed and cleanup timer restarted');
  }
  ...
  private async ensureBrowserLocked(): Promise<Browser> {
    if (this.browser && this.browser.isConnected()) return this.browser;
    return this.launchBrowserLocked();
  }

  private async launchBrowserLocked(): Promise<Browser> {
    if (this.browser) {
      this.browserCrashes++;
      logger.warn({ crashes: this.browserCrashes }, 'BrowserManager: Browser disconnected — cleaning up and relaunching');
      for (const ctx of this.contexts) {
        for (const page of [...ctx.pages]) {
          try { await page.close(); this.totalPagesClosed++; } catch {}
        }
      ...
      this.contexts = [];
      try { await this.browser.close(); logger.info({ crashes: this.browserCrashes }, 'BrowserManager: Crashed browser closed'); } catch {}
      this.browser = null;
    }
    ...
    if (now - this.lastLaunchAttemptTime > this.backoffResetWindowMs) {
      ...
      logger.debug({}, 'BrowserManager: Launch attempt counter reset after cooldown');
    }
    ...
    if (this.launchAttempts > this.maxLaunchAttempts) {
      ...
      throw new Error(
        `BrowserManager: Browser launch failed ${this.maxLaunchAttempts} times. ` +
    ...
    if (!existsSync(PERSISTENT_PROFILE_DIR)) mkdirSync(PERSISTENT_PROFILE_DIR, { recursive: true });
    ...
    try {
      const browserContext = await chromium.launchPersistentContext(PERSISTENT_PROFILE_DIR, {
  ...
  private async findOrCreateContextLocked(): Promise<ManagedContext> {
    for (const ctx of this.contexts) {
      if (ctx.pages.size < MAX_PAGES_PER_CONTEXT) return ctx;
    }
  ...
  private async setupPage(page: Page): Promise<void> {
    const diagnostics: PageDiagnostics = { console: [], requests: [] };
    ...
    page.on('console', (message) => {
      try {
        ...
        logger.warn({ err: err instanceof Error ? err.message : String(err) }, 'BrowserManager: console handler error');
      }
    });
    ...
    page.on('request', (request) => {
      try {
        ...
        logger.warn({ err: err instanceof Error ? err.message : String(err) }, 'BrowserManager: request handler error');
      }
    });
    ...
    page.on('requestfinished', async (request: Request) => {
      try {
        ...
        logger.warn({ err: err instanceof Error ? err.message : String(err), url: request.url() }, 'BrowserManager: requestfinished handler error');
      }
    });
    ...
    page.on('requestfailed', (request) => {
      try {
        ...
        logger.warn({ err: err instanceof Error ? err.message : String(err) }, 'BrowserManager: requestfailed handler error');
      }
    });
    ...
    await page.route('**/*', async (route) => {
      try {
        ...
        if (BLOCKED_RESOURCE_TYPES.has(resourceType)) { await route.abort(); return; }
        for (const domain of BLOCKED_DOMAINS) {
          if (url.includes(domain)) { await route.abort(); return; }
        }
    ...
    this.cleanupTimer = setInterval(async () => {
      ...
      try {
        if (this.browser && !this.browser.isConnected()) {
          logger.warn({}, 'BrowserManager: Browser disconnected during cleanup');
          this.browser = null;
        }
        const now = Date.now();
        for (let i = this.contexts.length - 1; i >= 0; i--) {
          ...
          if (ctx.pages.size === 0 && (now - ctx.lastUsed) > BROWSER_IDLE_TIMEOUT_MS) {
            ...
            logger.debug({ remainingContexts: this.contexts.length }, 'BrowserManager: Idle context cleaned');
          }
        }
      } finally {
        unlock();
